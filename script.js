...
let isDisplay = false;
const mole = document.querySelectorAll('.hole-container')[number];

function hideMole(number) {
  isDisplay = false;
  const mole = document.querySelectorAll('.hole-container')[number];
  mole.classList.remove('come');
}

function feedMole(number) {
  hideMole(number);
  number = Math.floor(Math.random()*10);
  return number;
}

function showHungryMole(number) {
  isDisplay = true;
  const mole = document.querySelectorAll('.hole-container')[number];
  mole.classList.add('come');
  number = mole.addEventListener('click', function(event) {
    return feedMole(number);
  });
}
 */
...
/* function next(number) {
  console.log(document.querySelectorAll('.hole-container')[number])
  let moleIsDisplayed = (document.querySelectorAll('.hole-container')[number].classList.length>1);
  if (Date.now() > nextTime) {
    if (moleIsDisplayed) {
      hideMole(number);
    } else {
      showHungryMole(number);
    }
...
/* function next() {
  if (Date.now() > nextTime) {
    ...
    nextTime = Date.now() + 1000 + (Math.random()*1000);
  }
  requestAnimationFrame(next);
}
